...
fn write(context: FormatContext, writer: anytype) @TypeOf(writer).Error!void {
    ...
    for (builder.files.keys(), builder.files.values()) |file_name, file| {
        ...
        while (it.next()) |line_messages| {
            ...
            if (last_line_end_with_unified < unified_loc.start) { // no intersection
                ...
                try writer.writeAll(file.source[last_line_end..line_loc.start -| 1]);
                try writer.writeAll(file.source[line_loc.start..line_loc.end]);
            }
            ...
            for (line_messages) |item| {
                ...
                if (context.tty_config) |tty| try tty.setColor(writer, color);
                try writer.print(" {s}: ", .{level_txt});
                if (context.tty_config) |tty| try tty.setColor(writer, .reset);
                try writer.writeAll(item.message);
            }

            if (it.msg_index != it.messages.len) {
                try writer.writeByte('\n');
            }
        }
